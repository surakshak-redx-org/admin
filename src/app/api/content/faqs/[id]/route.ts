...
export async function PUT(request: NextRequest, context: RouteContext): Promise<Response> {
  ...
  try {
    ...
    await updateContent(COLLECTIONS.FAQS, id, body);
    return apiOk({ id });
  } catch (error) {
    console.error('PUT /api/content/faqs/[id] failed:', error);
...
export async function DELETE(request: NextRequest, context: RouteContext): Promise<Response> {
  ...
  try {
    const { id } = await context.params;
    await deleteContent(COLLECTIONS.FAQS, id);
    return apiOk({ id });
  } catch (error) {
    console.error('DELETE /api/content/faqs/[id] failed:', error);
...
export async function PATCH(request: NextRequest, context: RouteContext): Promise<Response> {
  ...
  try {
    ...
    return apiError('Failed to toggle FAQ', 500);
  }
}
